...
[구현 KG 기록](ontology/evidence/HSWM_ICE_GRAPH_IMPLEMENTATION_WP0_3_2026-09-08.v1.json)은
소스·검사·작업 의존성과 미구현 후속을 구분해 연결한다.
...
`60 * tail <= 2^m`으로 판정한다. 16개 Lean 정리는 tie count가 exact gate를 바꾸지 않음,
zero-discordance 실패, all-win `m=5` 실패와 `m=6` 통과, exact arithmetic 실패가 downstream
scientific GO를 막음을 증명한다. 또한 frozen `analysis_v1.json`의 small-tail, all-tie,
300-all-win, 220-win primary, `(300,180)` nontrivial tail과 LCB/exact split count projection을
...
control-core blocker, 여전히 열린 live-adapter/three-job chronology gate,
보호 파일과 다음 실행 순서는
...
서비스나 개인 거버넌스 계층이 아니라 각 행의 체크인 증거와 명시된 외부 provenance
경계에서 읽는다. SWM-0W receipt의 독립 권위는 live GitHub evidence 재조회가
필요하다. 활성 기계 정책은
...
`hswm-swm0w-protocol/v2`는 20-task 결과를 candidate-only로 환원한다. 미래 Quicknet
round를 source-freeze 뒤 preregister한 단일 GitHub run은
`CANDIDATE_PASS_AWAITING_BUNDLE`을 냈고, 별도 adjudicator가 GitHub chronology와 exact
artifact, pinned-Node BLS, 20개 task/receipt/reducer를 재검증한 뒤에만 evidence `PASS`로
승격했다. 따라서 fixed-three-singleton scalar precursor만 `SUPPORTED_NARROW`다. 이는
multi-member set aggregation, recipient-specific set-to-set transport, recurrent
depth, causal `ΔW`, 또는 HSWM 전체 효능의 구현·증거가 아니다.
...
1. 완료: 3-job future-beacon 경계의 SWM-0W scalar candidate를 sole-surviving same-head GitHub run으로 실행하고 외부 provenance로 adjudicate했다. candidate-only 결과는 별도 replay 뒤 `PASS`, bounded claim은 `SUPPORTED_NARROW`로 보존했다. GitHub control plane과 matching-run 미삭제는 여전히 명시적 운영 신뢰 경계다.
2. Multi-member, recipient-specific set-to-set `W`의 finite world, operator core, V2 generator와 deterministic optimizer를 구현했고 disclosed train/dev pilot으로 세 learning rate를 adoption bundle에 고정했다. outcome-independent resource policy와 TypeScript/Effect pure control core의 독립 감사도 통과했다. 다음에는 live Git/GitHub/drand/Python/artifact adapter와 durable `register -> confirm -> adjudicate` chronology를 구현·독립감사한다. 그 뒤에만 단일 future-seed chronology를 source A에 동결하고 direct-child prereg 뒤 새 matched gate로 시험한다. V2 draw를 독립 mechanism으로 부르거나 scalar `PASS`·constructive Q witness·epoch-zero DS comparator를 learned S2S 성공/경쟁력 문구로 재사용하지 않는다.
