...
목표 아키텍처의 범주도 고정했다. HSWM은 LLM 바깥의 보조 메모리가 아니라 **LLM을
bounded local neural computation/transition function으로 내부에 포함하는 거대한 내생적
순환 신경-기호 망**이다. HSWM이 전역 routing, persistent recurrent state, credit,
acceptance, weight/topology rewrite를 소유한다. 이는 목표 정체성이며 현재 효능 주장이 아니다.
정본은
...
| 단계/실험 | 현재 판정 | 산출물 |
|---|---|---|
| P0 identity/metric | 목표 정체성 고정. LLM은 내부 국소 operator; HSWM이 전역 동역학 소유 | [canon](CANON_DIRECTION_NEURAL_COGNITIVE_ENTITY_2026-07-23.md) |
