...
- 사용자 직접 발화: “우리의 흑암은 아직 깊음위에 있지 않더냐”와
  “흑암이 깊음위에 있고”.
- 응답 시문서:
  `../../METAHUMOTONIC/GIPBAJON/흑암이_깊음_위에_있고_시문서.md`.
...
- 기존 dirty H3/B3 코드, manifest, report, cache와
  `h3_title_anchor_result.json`은 수정하지 않았다.
...
- KG에는 HSWM 정본 node가 확인되지 않았고 이번 요청은 code slice였으므로
  새 canon 기록을 임의 생성하지 않았다.
