...
PLAN_SCHEMA = "hswm-dgx-qcase024-mi-plan/v1"
MARKER_SCHEMA = "hswm-dgx-qcase024-mi-start-marker/v1"
NAMESPACE = "DNRD5-QCASE024-MECHANISM-ISOLATION-ONLY/v1"
RUNNER_VERSION = "hswm-dgx-qcase024-mi-runner/v1"
FREEZE_SCHEMA = "hswm-dgx-qcase024-mi-preregistration-freeze/v1"
REGISTRY = {
    "schema_version": "hswm-dgx-qcase024-mi-plan-consumption-registry/v1",
    "path": "/mnt/hswm/evidence/hswm-dnrd5-qcase024-mi-1-consumption-v1",
...
_ATTEMPT = re.compile(r"^MI-024-(ASYNC_ENABLED|ASYNC_DISABLED)-B0[12]-R00[1-4]$")
...
def validate_arm_identities(identities: Mapping[str, Mapping[str, bytes]]) -> None:
    ...
    for arm in ARMS:
        ...
        if (set(runtime) != runtime_keys
                or runtime.get("schema_version") != "hswm-dgx-qcase024-mi-runtime-identity/v1"
...
def validate_mi_plan(raw: bytes) -> dict[str, Any]:
    ...
    attempts = [f"MI-024-{arm}-{block}-R{rep:03d}" for arm, block in BLOCKS for rep in range(1, 5)]
...
def make_mi_start_marker(plan_raw: bytes) -> bytes:
    ...
    return canonical_bytes({"schema_version": MARKER_SCHEMA, "namespace": NAMESPACE,
        ...
        "scheduled_attempts": [f"MI-024-{arm}-{block}-R{rep:03d}" for arm, block in BLOCKS for rep in range(1, 5)],
