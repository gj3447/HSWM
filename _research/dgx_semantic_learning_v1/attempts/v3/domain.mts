...
export const models = [{ id: 'qwen3-4b-real', port: 8001 }] as const;
...
export const protocol = {
  ...
  trials: 16, maximum_model_calls: 128, maximum_wall_seconds: 2400, trial_concurrency: 1,
  amendment: 'v1/v2 retained. v3 evaluates the healthy 4B service with JSON Schema constrained output. Native relation, labels, splits, scoring and admission unchanged. This is a new format-controlled condition, not replacement of previous failures or an estimated model-size comparison.',
...
/** The constraint contains shape only, never target labels or relation truth. */
export function responseFormat(request: { contract: string; frame: { event: string; relation: { semantic: { exceptionRefs: string[] } } } }) {
  const predict = request.contract === 'hswm-llm-semantic-predict/v1';
  const properties = predict ? {
    prediction: { type: 'string', pattern: `^[01]{${JSON.parse(request.frame.event).cases.length}}$` },
    uncertainty: { type: 'string', minLength: 1, maxLength: 8192 }
  } : {
    semanticText: { type: 'string', minLength: 1, maxLength: 8192 },
    disposition: { type: 'string', minLength: 1, maxLength: 8192 },
    uncertainty: { type: 'string', minLength: 1, maxLength: 8192 },
    exceptionRefs: { type: 'array', const: request.frame.relation.semantic.exceptionRefs }
  };
  return { type: 'json_schema', json_schema: { name: predict ? 'hswm_prediction' : 'hswm_revision', strict: true,
