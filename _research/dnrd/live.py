...
def _candidate_response_tokens(request: ModelRequest) -> tuple[str, str]:
    """Read DNRD-4S1's per-episode, two-token output contract.
    ...
    if type(candidates) is not tuple or len(candidates) != 2:
        raise PreDispatchAnswererError("DNRD-4S1 request must carry exactly two candidate response tokens")
    ...
    if (
        ...
        raise PreDispatchAnswererError(
            "DNRD-4S1 request candidate response tokens must be distinct canonical response tokens in sorted order"
        )
    return first, second
...
def _strict_structured_response(content: str, candidates: tuple[str, str]) -> str:
    ...
    if not is_response_token(response_token):
        raise LiveBoundaryError("chat completion structured response_token violates exact DNRD-4S1 form")
