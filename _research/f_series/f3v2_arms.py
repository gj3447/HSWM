...
* (g) flat_file   — §6.5 strong null (AutoMEM-style flat-file 자기관리):
                    the SAME donor train lessons as (b) naive, rendered as
                    ONE unstructured flat notes file (no typed kind tags, no
                    lesson ids) plus a self-management instruction line.  It
                    isolates the typed-structure contribution: vs (b) it
                    varies presentation/management only, vs (c)/(d) it holds
                    content constant — typed arms that cannot beat it have
                    structural contribution 0.  Gate-invariant; a scored arm
                    but NOT part of the min(TRR_c, TRR_d) judgment target
                    (its TRR rides alongside in the table).
...
GATED_ARM_IDS = ("b_naive_donor_gated", "c_abstracted_gated",
                 "d_contrast_gated")
FLAT_FILE_ARM = "g_flat_file"
# prereg budget_contract: 10 contexts per test world =
# 6 core (a-f) + 1 flat-file strong null (g) + 3 gated variants (b/c/d)
SEALED_CONTEXTS = (CORE_ARM_IDS + (FLAT_FILE_ARM,) + GATED_ARM_IDS)
ARM_POOLS = {"b_naive_donor": "naive", "c_abstracted": "abstracted",
             "d_contrast": "contrast", "e_b_self": "b_self",
             "f_placebo": "placebo", FLAT_FILE_ARM: "naive"}
GATE_INVARIANT_ARMS = ("a_no_memory", "e_b_self", "f_placebo", FLAT_FILE_ARM)

MANIFEST_SCHEMA = "hswm-f3v2-sealed-manifest/v1"
...
FLAT_FILE_INSTRUCTION = (
    "Below is your notes file from previous work. Use whatever parts you "
    "judge useful; ignore the rest.")


def render_flat_file(lessons: list[dict]) -> str:
    """(g) flat_file strong null (prereg §6.5, AutoMEM-style): the SAME
    lessons as (b) naive, rendered as ONE unstructured flat notes document —
    no typed kind tags, no lesson ids, no numbered structure; paragraphs of
    raw lesson text under the self-management instruction line."""
    paras = [FLAT_FILE_INSTRUCTION]
    for lesson in lessons:
        if lesson["kind"] in ("norm", "contrast"):
            body = f"{lesson['enforce']} {lesson['avoid']}"
        else:
            body = lesson["text"]
        paras.append(" ".join(str(body).split()))
    return "\n\n".join(paras)


def _sparse(vec: list[float]) -> list[tuple[int, float]]:
    return [(i, v) for i, v in enumerate(vec) if v]


def _sparse_dot(dense: list[float], sparse: list[tuple[int, float]]) -> float:
    return sum(dense[i] * v for i, v in sparse)


def _parse_arm_id(arm_id: str) -> tuple[str, bool]:
    if arm_id == "x_xvendor":
    ...
    if base_arm != "a_no_memory" and base_arm not in ARM_POOLS:
        raise ArmsError(f"unknown arm {arm_id!r}")
    return base_arm, gated


def arm_prompt_builder(arm_id: str, lesson_sets: dict, *, embedder=None,
                       top_k: int = TOP_K):
    """world -> augmented prompt for one arm, with the pool filtering and
    lesson embeddings precomputed ONCE (sealed scale: every test world reuses
    the same vectors).  Deterministic: (score desc, lesson_id asc)."""
    embedder = embedder or HashEmbedder()
    base_arm, gated = _parse_arm_id(arm_id)
    if base_arm == "a_no_memory":
        return fw.render_prompt
    lessons = lesson_sets[ARM_POOLS[base_arm]]
    if gated:  # gate ON: only disagreement-flagged lessons transfer
        lessons = [lesson for lesson in lessons if lesson["flagged"]]
    vecs = [(lesson, _sparse(embedder(_lesson_embed_text(lesson))))
            for lesson in lessons]
    flat = base_arm == FLAT_FILE_ARM

    def prompt(world: dict) -> str:
        base = fw.render_prompt(world)
        if not vecs:
            return base
        q = embedder(_rules_text(world))
        scored = sorted(((_sparse_dot(q, sv), lesson["lesson_id"], lesson)
                         for lesson, sv in vecs),
                        key=lambda s: (-s[0], s[1]))
        top = [lesson for _s, _lid, lesson in scored[:top_k]]
        if not top:
            return base
        block = render_flat_file(top) if flat else render_lesson_block(top)
        return block + "\n\n" + base
...
def build_arm_prompt(arm_id: str, world: dict, lesson_sets: dict, *,
    ...
    return arm_prompt_builder(arm_id, lesson_sets, embedder=embedder,
                              top_k=top_k)(world)
...
def evaluate_arms(chat, model: str, test_worlds: list[dict],
    ...
    out = {}
    for arm in arm_ids:
        builder = arm_prompt_builder(arm, lesson_sets, embedder=embedder)
        rows = run_experience(chat, model, test_worlds, seed=seed,
...
# ---------------------------------------------------------------- sealed cohort
def cohort_from_manifest(manifest: dict) -> dict:
    """Regenerate the manifest cohort deterministically (prereg seeds/sizes/
    tiers) and verify every world's sha256 against the manifest list — the
    manifest IS the binding; a mismatch means generator/seed drift, fail
    closed.  Returns {"train": [...], "test": [...]} (mixed tiers)."""
    if manifest.get("schema_version") != MANIFEST_SCHEMA:
        raise ArmsError(f"not a {MANIFEST_SCHEMA} manifest")
    out = {}
    for split in ("train", "test"):
        block = manifest["cohort"][split]
        worlds = []
        for tier in sorted(block["tiers"]):
            worlds.extend(fw.generate_batch(block["tiers"][tier], tier,
                                            block["seed"]))
        expected = [w["world_sha256"] for w in block["worlds"]]
        actual = [fw.world_sha256(w) for w in worlds]
        if actual != expected:
            raise ArmsError(
                f"cohort regeneration mismatch vs manifest ({split}): the "
                "generator module or seeds drifted — the sealed cohort "
                "cannot be reproduced; run void")
        out[split] = worlds
    return out
...
def build_receipt(*, script_path, config: dict, train_worlds: list[dict],
                  ...
                  prereg_path=PREREG_PATH, mode: str = "development",
                  stage: str | None = None,
                  preregistration_receipt_sha256: str | None = None,
                  extra: dict | None = None) -> dict:
    """Canary-gate-convention receipt: hswm-f3v2-arms-receipt/v1.

    mode="sealed" only for a sealed-manifest run; stage defaults to SEALED
    for a live sealed run and stays DEVELOPMENT_ONLY otherwise (a scripted
    dry-run of a sealed manifest keeps mode="sealed" for config binding but
    stage DEVELOPMENT_ONLY — scripted numbers never carry sealed weight).
    preregistration_receipt_sha256 binds the machine-locked prereg JSON
    (manifest-carried), distinct from preregistration_file_sha256 (the MD
    doc)."""
    ...
    receipt = {
        ...
        "mode": mode,
        "stage": stage or ("SEALED" if mode == "sealed"
