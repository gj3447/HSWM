...
PREP") is filled REPO_ROOT in the manifest — the prereg file itself stays
...
Usage:
  python3 -m _research.f_series.f3v2_sealed_prep \
  ...
  python3 -m _research.f_series.f3v2_sealed_prep --seal \
...
DEFAULT_PREREG = (REPO_ROOT / "prom_search_hswm" / "evidence"
...
def build_manifest(prereg: dict, *, prereg_path: str,
    ...
    generator_sha = _sha256_path(source_path("f3v2_procedural_worlds.py"))
    return {
        ...
        "generator": {
            ...
            "note": ("fills prereg environment.generator_sha256_at_lock "
                     ...
                     "locked prereg file; the value is sealed REPO_ROOT)."),
        },
        "harness_module_sha256": {
            name: _sha256_path(source_path(name)) for name in HARNESS_MODULES},
