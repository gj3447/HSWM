...
project_root=/data/kjra/PROJECT/HSWM_P1V2_20260724_FROZEN_DATA
final_root="$project_root/phantomwiki_seed2/sparse_t200_fk1"
staging_root="$project_root/.staging-phantomwiki-seed2-sparse-t200-fk1"
...
if [[ -e "$staging_root" ]]; then
  if [[ -s "$staging_root/articles.json" && -s "$staging_root/questions.json" ]]; then
    resume_staging=true
  else
    echo "refusing ambiguous pre-existing staging path: $staging_root" >&2
    exit 42
  fi
fi

mkdir -p "$tool_root/bin" "$project_root/phantomwiki_seed2"
if [[ ! -x "$micro_bin" ]]; then
  archive=$(mktemp)
  ...
  curl --fail --location --silent --show-error \
    ...
    --output "$archive"
  tar -xjf "$archive" -C "$tool_root" bin/micromamba
fi

if [[ ! -x "$env_root/bin/python" ]]; then
  "$micro_bin" create --yes --override-channels --prefix "$env_root" \
    --channel conda-forge python=3.12 swi-prolog=9.2.9 pip
fi

if ! "$env_root/bin/python" -c 'import importlib.metadata as m; assert m.version("phantom-wiki") == "1.0.3"' 2>/dev/null; then
  "$env_root/bin/python" -m pip install --disable-pip-version-check \
    phantom-wiki==1.0.3
fi

"$env_root/bin/swipl" --version
"$env_root/bin/python" -c 'import importlib.metadata as m; print("phantom-wiki", m.version("phantom-wiki"))'
...
"$env_root/bin/python" - "$final_root" "$project_root/generation_receipt_seed2.json" <<'PY'
from __future__ import annotations
...
receipt = {
    "schema_version": "hswm-p1v2-phantom-generation-receipt/v1",
    "generator": {
