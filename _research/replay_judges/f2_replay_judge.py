"""HSWM_LOCAL_RECORD-compatible replay judge for the F2 sealed measurement.
...
def replay_metric(receipt_path: Path) -> float:
    ...
    harness_path = F_SERIES_ROOT / "f2_delta_w_credit.py"
    if not harness_path.is_file():
        raise F2ReplayError("canonical F2 harness script missing")
    ...
    h = importlib.import_module("_research.f_series.f2_delta_w_credit")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1 or args[0].startswith("-"):
        print(
            "replay_error=usage: python -m _research.replay_judges.f2_replay_judge <receipt_path>",
            file=sys.stderr,
        )
        return 1
    try:
