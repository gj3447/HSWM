"""HSWM_LOCAL_RECORD-compatible replay judge for the F3-r3 sealed measurement.
...
def _harness_module_path(relative_name: str) -> Path:
    if relative_name in {
        "f2_delta_w_credit.py",
        "f3_agent_ab_transfer.py",
        "f3_agent_ab_transfer_r2.py",
    }:
...
def replay_metric(receipt_path: Path) -> float:
    ...
    harness_path = F_SERIES_ROOT / "f3_agent_ab_transfer_r3.py"
    if not harness_path.is_file():
        raise F3ReplayError("canonical f3-r3 harness script missing")
    ...
    f3r1 = importlib.import_module("_research.f_series.f3_agent_ab_transfer")
    f3r2 = importlib.import_module("_research.f_series.f3_agent_ab_transfer_r2")
    m = importlib.import_module("_research.f_series.f3_agent_ab_transfer_r3")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1 or args[0].startswith("-"):
        print(
            "replay_error=usage: python -m _research.replay_judges.f3_replay_judge <receipt_path>",
            file=sys.stderr,
        )
        return 1
    try:
