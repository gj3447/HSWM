...
def replay_metric(receipt_path: Path) -> float:
    ...
    harness_path = F_SERIES_ROOT / "f5_consolidation.py"
    if not harness_path.is_file():
        raise F5ReplayError("canonical f5 harness script missing")
    ...
    if module_sha:
        p = F_SERIES_ROOT / "f2_delta_w_credit.py"
    ...
    m = importlib.import_module("_research.f_series.f5_consolidation")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1 or args[0].startswith("-"):
        print(
            "replay_error=usage: python -m _research.replay_judges.f5_replay_judge <receipt_path>",
            file=sys.stderr,
        )
        return 1
    try:
