...
> LLM-executed function cell들이 그 안에서 작동하고, 관계·가중치·활성·기억·함수의
> evolving hypergraph가 곧 지속되는 몸이자 월드모델이 되며, outcome에 결속된
> continuous learning으로 자신의 `W`, routing, `H`를 바꾸는 자기기술적 living
> harness다.**
...
2. token signal은 `A_t`를 점화하고 `H_t/W_t`를 따라 function cell의 의미 전이를 유발한다.
3. 출력 token과 외부 action은 다시 세계의 outcome과 결속될 수 있는 trajectory가 된다.
4. 그 trajectory의 credit만이 durable `W/H/routing` update 후보가 된다.
...
- 한 시점의 세계는 객체 목록이 아니라 typed node와 n-ary hyperedge의 관계 상태다.
- 변화는 기존 세계 밖에서 최종 문서를 교체하는 것이 아니라, 상태 안에서 일어나는
  국소 rewrite/update event다.
...
인과 계보**라는 존재론적 역할을 고정한다. 어떤 rewrite를 선택할지는 LLM token activation,
외부 outcome, credit, 검증, 헌법 경계를 가진 HSWM 고유의 문제다.
...
typed token/action/tool trajectory
  ...
  → activation eligibility와 causal credit
  → bounded ΔW / Δrouting / ΔH candidate
  → fresh·retention·canary·removal validation
  → accept 또는 reject/rollback
  → S_(t+1)의 달라진 행동
```
...
학습 대상은 특정 prompt rule의 증식이 아니라 다음을 포함한다.

- 어떤 기억·함수·도구·검증기를 어떤 문맥에서 활성화하거나 억제할지
- 어떤 cell들이 coalition을 이루고, 어떤 relation으로 message를 전달할지
- 언제 retry, stop, recovery, specialization이 유효한지
- 어떤 관계를 강화·약화하고, 언제 `H`를 연결·분리·전문화할지

`Π`의 권한·안전 기준 자체를 임의로 학습해 우회하는 것은 continuous learning이 아니라
정체성 파괴다.
...
2. 자기 상태를 실행 규칙이자 가독 문서로 투사하는 living harness
3. rewrite되는 관계 세계 안에서 인지·행동하는 hypergraph world model
4. outcome이 미래의 배선과 행동을 바꾸는 continuous learner
