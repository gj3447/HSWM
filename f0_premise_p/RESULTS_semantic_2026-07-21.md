# F0 의미지표 결과 — HSWM_LOCAL_RECORD 판정 (2026-07-21)

> 후속: token-F1이 아티팩트였던 F0([`RESULTS_vllm_2026-07-21.md`](RESULTS_vllm_2026-07-21.md))에 **독립 의미지표** + **HSWM_LOCAL_RECORD 결정론 judge**.
> tree=`HSWM_LOCAL_RECORD` · tag=`f0-premp-semantic-2026-07-21` · evidence=`EVIDENCE_semantic_embedding_2026-07-21.json`
...
judge가 과대주장을 막음:
1. **`script_sha_server_verified: false`** — novel_script가 Mac 로컬 경로, HSWM_LOCAL_RECORD 서버는 원격(airo)이라 파일 못 읽음 → novel *서버앵커* 미성립(`novel_unconfirmed`).
2. **eureka `hallucinated`** — `closes_question` 미선언 → problem_balance 0 + BF marginal(1.0).
3. **legacy_program `provisional_stale_engine`** — 정성 영수증 미완.
...
- **단 HSWM_LOCAL_RECORD는 partial** — 완전 인증 아님. 이게 정직의 핵심: 강한 신호지만 (서버앵커+정성영수증+실데이터) 미완이라 judge가 progressive를 안 줌.
...
3. **`closes_question` 선언** + 정성 Legacy programme 영수증 → eureka 실화 + full progressive 가능.
...
# HSWM_LOCAL_RECORD: tree=HSWM_LOCAL_RECORD tag=f0-premp-semantic-2026-07-21
```
