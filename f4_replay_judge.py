...
def replay_metric(receipt_path: Path) -> float:
    ...
    r2_path = HERE / "f4_topology_learning_r2.py"
    if not r2_path.is_file():
        raise F4ReplayError("r2 harness script missing beside the judge")
    ...
    for rel, sha in module_shas.items():
        ...
        if not p.is_file():
            null_battery = Path(
                "/Users/lagyeongjun/CD/SYMPOSIUM/HSWM/prom_search_hswm") / rel
            p = null_battery
    ...
    m = importlib.import_module("f4_topology_learning_r2")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1 or args[0].startswith("-"):
        print("replay_error=usage: f4_replay_judge.py <receipt_path>", file=sys.stderr)
        return 1
    try:
