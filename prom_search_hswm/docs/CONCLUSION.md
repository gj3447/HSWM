...
>
> **답 (한 줄)**: **네 비전의 검색층 부분은 REAL이고 검색 SOTA(RAPTOR+GraphRAG+SiReRAG)와 정확히 일치한다. 단 세 개의 딱딱한 조건이 붙는다 — 깊이는 얕게(3~5), 수평엣지는 선별적·multi-hop서만, 가중은 필수. "10-level·무조건 많이"는 실험·문헌 둘 다 반박한다.** 근거 = 실험(ML1-9) + 문헌 5편 + 적대적 설계검증, 세 축이 수렴.
