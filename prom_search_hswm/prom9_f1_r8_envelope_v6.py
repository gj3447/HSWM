#!/usr/bin/env python3
"""Derive the tight common token envelope for the F1 r8 C-recontract (v6).

Fork of the v4 derivation for the cohort-expansion generation ratified
2026-08-05: the selection receipt is a v6 receipt (800/800 train cohorts),
cohort scales come from that policy instead of frozen 54/100/100 constants,
and the manifest rows carry the selection's own dataset split.  Everything
else — tokenizer identity,
...
The derivation receipt binds the actual v6 selection self-hash; freezing
...
DEVELOPMENT_RUN_ID_C801 = C801_DEVELOPMENT_RUN_ID
CONFIRMATORY_RUN_ID_C801 = C801_SEALED_RUN_ID
DATASET_C801 = "framolfese/2WikiMultihopQA"
DATASET_CONFIG_C801 = "default"

# The v6 selection receipt embeds every redacted pool page (450 pages for the
...
# verify_selection_receipt_v6 after parsing.
...
def _read_bound_selection_json(
    ...
    """`_read_bound_json` with the v6 selection-receipt byte ceiling."""
...
def build_token_envelope_artifacts_v6(
    *,
    selection: Mapping[str, object],
    ...
    model: str,
    model_revision: str,
    development_run_id: str = DEVELOPMENT_RUN_ID_C801,
    confirmatory_run_id: str = CONFIRMATORY_RUN_ID_C801,
    token_tolerance: int = DEFAULT_TOKEN_TOLERANCE,
) -> dict[str, dict[str, object]]:
    """Build the c801 common envelope and its outcome-blind derivation receipt."""
    ...
    for cohort, run_id in (
        ...
        items, _summary = _cohort_items_v6(
            selection=selection,
    ...
    for cohort, run_id in (
        ...
        items, summary = _cohort_items_v6(
            selection=selection,
            selection_sha256=selection_sha,
            cohort=cohort,
            run_id=run_id,
    ...
    receipt_unsigned = {
        ...
        "selection_generation": "hswm-prom9-f1-r8-cohort-selection/v6",
        "dataset_split": dataset_split,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--development-run-id", default=DEVELOPMENT_RUN_ID_C801
    )
    parser.add_argument(
        "--confirmatory-run-id", default=CONFIRMATORY_RUN_ID_C801
    )
    parser.add_argument(
    ...
    try:
        selection, selection_file_sha = _read_bound_selection_json(
            args.selection_receipt, "v6 selection receipt"
        )
        ...
        artifacts = build_token_envelope_artifacts_v6(
            selection=selection,
