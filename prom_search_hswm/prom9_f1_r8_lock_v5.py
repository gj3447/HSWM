#!/usr/bin/env python3
"""Build the gold-blind execution lock for the r8 c800 development run.

Generation-bound fork of ``prom9_f1_r8_lock`` for the C-recontract cohorts:
the selection input is the ~240 MB cohort-selection/v5 receipt (dedicated
large stable-read ceiling), replay goes through
``replay_selection_receipt_v5`` (which also verifies and lifts the pinned
predecessor selection), the run identity is the ratified
``f1-2wiki-development-r8-c800``, and the token-envelope derivation is
verified by full deterministic REBUILD via
``build_token_envelope_artifacts_v5`` instead of the v4 verify path.  The
...
def _validate_artifact_graph(
    *,
    ...
    # The v4 accessor re-verifies the v3/v4 selection schemas and refuses v5;
    # the caller has already fully replayed the v5 receipt, so read the
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        selection, _selection_file_sha = _read_bound_selection_json(
            args.selection_receipt, "v5 cohort selection receipt"
        )
        predecessor, _predecessor_file_sha = _read_with_sha(
        ...
        if (
            selection.get("schema_version") != SELECTION_SCHEMA_V5
            or manifest.get("run_id") != DEVELOPMENT_RUN_ID_C800
        ):
            raise LockRefusal(
                "c800 development lock requires the v5 selection and the "
                "ratified c800 run identity"
            )
