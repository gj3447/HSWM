...
export const nativeS2SParameterSha256 = (parameters: NativeS2SParameters, arm: NativeS2STrainingArm): string => { const hash = createHash("sha256").update("hswm-swm0w-s2s-parameters/v1\0"); const record = parameterRecord(parameters); for (const [key, pythonName, shape] of [...names(arm)].sort((left, right) => left[1].localeCompare(right[1]))) {
