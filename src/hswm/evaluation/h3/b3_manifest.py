...
This is the canonical evaluation module; the legacy top-level import is a
compatibility alias.
...
def _discover_repository_root(anchor: str | Path = __file__) -> Path:
    """Find the source checkout root without depending on module depth."""
    ...
    for candidate in (start, *start.parents):
        if all((candidate / marker).is_file() for marker in _REPOSITORY_MARKERS):
            return candidate
    raise RuntimeError(
        f"cannot discover HSWM repository root from {resolved}; "
        f"missing markers {_REPOSITORY_MARKERS!r}"
    )
