...
from _research.dgx_mi.independent_verifier import (
    COMPLETE, INCOMPLETE, UNAVAILABLE, VOID, _ordinary, _tokens,
    _trace_projection, _usage_v4, main, verify,
)
...
def _envelope(content: str, *, peer: bool = True) -> bytes:
    ...
    for character in content.encode():
        ...
        top += [{"token": f"alt-{index}", "bytes": [(character + index) % 256],
                 "logprob": -1.1 - index / 100} for index in range(1, 20)]
        tokens.append({"token": chr(character), "bytes": [character], "logprob": -0.1, "top_logprobs": top})
    terminal = b"<|im_end|>"
    terminal_top = [{"token": "<|im_end|>", "bytes": list(terminal), "logprob": 0.0}]
    terminal_top += [{"token": f"alt-{index}", "bytes": [index],
                      "logprob": -1.1 - index / 100} for index in range(1, 20)]
    tokens.append({"token": "<|im_end|>", "bytes": list(terminal), "logprob": 0.0,
                   "top_logprobs": terminal_top})
...
def _run_valid(tmp_path: Path, *, variant: bool = False, short_top: bool = False, early_failure: bool = False,
               nullable_usage_detail: bool = False, trace_fault: str | None = None):
    ...
    content = '{\n  "rationale": "The first public cue begins with V, while the other cue is a different public word and label.",\n  "answer": "VISTA"\n}'
    ...
    if trace_fault is not None:
        body = json.loads(raws[0]); rows = body["choices"][0]["logprobs"]["content"]
        if trace_fault == "missing": rows.pop()
        elif trace_fault == "wrong_text": rows[-1]["token"] = "<|wrong|>"
        elif trace_fault == "wrong_bytes": rows[-1]["bytes"][-1] = 33
        elif trace_fault == "duplicate": rows.insert(-1, dict(rows[-1]))
        elif trace_fault == "nonfinal": rows[-1], rows[-2] = rows[-2], rows[-1]
        elif trace_fault == "content_mismatch": rows[0]["bytes"] = [ord("[")]
        elif trace_fault == "duplicate_candidate": rows[0]["top_logprobs"][1] = dict(rows[0]["top_logprobs"][0])
        else: raise AssertionError("unknown test trace fault")
        raws[0] = json.dumps(body, separators=(",", ":")).encode()
    if early_failure:
    ...
    if variant:
        alternative = content.replace("begins with V", "starts with V")
...
def test_valid_frozen_runner_root_reduces_to_complete_with_decimal_trace(tmp_path: Path) -> None:
    ...
    first_terminal = next(json.loads(line) for line in (root / "mi_ledger.jsonl").read_bytes().splitlines()
                          if json.loads(line)["record_type"] == "TERMINAL")
    trace = json.loads((root / "content" / first_terminal["token_logprob_trace"]["sha256"]).read_bytes())
    assert trace[-1]["token"] == "<|im_end|>"
    assert trace[-1]["bytes"] == [60, 124, 105, 109, 95, 101, 110, 100, 124, 62]
...
def test_verifier_usage_reducer_accepts_closed_provider_shapes(usage: dict[str, object]) -> None:
    assert _usage_v4(usage) == {"prompt_tokens": 1, "completion_tokens": 1, "total_tokens": 2}
...
@pytest.mark.parametrize("fault", ("missing", "wrong_text", "wrong_bytes", "duplicate", "nonfinal", "content_mismatch", "duplicate_candidate"))
def test_terminal_or_semantic_alignment_fault_seals_unavailable_not_void(tmp_path: Path, fault: str) -> None:
    root, registry, _ = _run_valid(tmp_path, trace_fault=fault)
    assert verify(root, external_registry_root=registry)["terminal"] == UNAVAILABLE


def test_standalone_trace_projection_preserves_exact_high_precision_decimal() -> None:
    content = '{"answer":"VISTA"}'
    raw = _envelope(content).replace(
        b'"logprob":-0.1', b'"logprob":-0.12345678901234567890123456789', 2,
    )
    envelope = _ordinary(raw)
    _tokens(envelope, content.encode())
    trace = parse_canonical(_trace_projection(envelope["choices"][0]["logprobs"]["content"]))
    assert trace[0]["logprob"] == "-1.2345678901234567890123456789E-1"
