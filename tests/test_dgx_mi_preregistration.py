...
def _arm_identities() -> dict[str, dict[str, bytes]]:
    ...
    def row(async_value: bool) -> dict[str, bytes]:
        ...
        runtime |= {"schema_version": "hswm-dgx-qcase024-mi-runtime-identity/v2", "async_scheduling": async_value,
...
def _inputs() -> MiPreregistrationInputs:
    ...
    return MiPreregistrationInputs(
        ...
        root_genesis=canonical_bytes({"schema_version": "hswm-dgx-qcase024-mi-evidence-root-genesis/v2",
            "nonce_hex": "3" * 64, "purpose": "FRESH_SINGLE_USE_QCASE024_MI_CLOSURE_V2_EVIDENCE_ROOT",
...
def test_refuses_exact_selection_and_zero_genesis_drift() -> None:
    ...
    values["root_genesis"] = canonical_bytes({"schema_version": "hswm-dgx-qcase024-mi-evidence-root-genesis/v2", "nonce_hex": "0" * 64, "purpose": "FRESH_SINGLE_USE_QCASE024_MI_CLOSURE_V2_EVIDENCE_ROOT", "terminal": "GENESIS_BOUND_BEFORE_ANY_MI_LIVE_START"})
    with pytest.raises(MiFreezeRefusal): build_mi_preregistration(MiPreregistrationInputs(**values))
