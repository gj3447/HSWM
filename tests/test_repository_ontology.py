...
def test_python_root_migrations_are_pinned_and_root_count_only_decreases() -> None:
    data = load_repository_ontology()
    result = validate_checkout(data)
    assert result["python_root_migrations"] == 71
    assert len(list(ROOT.glob("*.py"))) == 77
    assert result["root_python_sha_locked"] == 67
    ...
    assert classification["observed_root_python_count"] == 77
    assert classification["counts"]["partition_total"] == 77
